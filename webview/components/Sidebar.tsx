...
export function Sidebar({ connected, currentView, navigate }: SidebarProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // painted, which is what react-hooks flags as a cascading render. Here
      // they are still the first thing that happens, so the spinner and the
      // cleared error appear exactly as before.
      setLoading(true);
      setError(null);
      try {
